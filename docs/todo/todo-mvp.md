...
- 設定画面の表示確認
  - [ ] `SettingsModal.vue` の背景・余白・スクロールが実機で崩れていないか確認する。
  - [ ] 背面が透ける場合のみ、既存デザインに合わせて最小差分で修正する。
- 常に最前面機能の実機確認
  - [ ] 上部バーのボタン表示がピン留めアイコンと視覚的に競合していないか確認する。
  - [ ] Wails 側の `ToggleAlwaysOnTop` が実機で期待通り動作するか確認する。
- Markdown / Rich エディタ仕様の明文化
  - [ ] Markdown を保存責務の中心にする方針をドキュメントへ反映する。
  - [ ] Rich は Markdown への変換可能範囲に限定する方針を明文化する。
  - [ ] raw HTML の扱いを仕様として明文化する。
- table 編集 UI の再設計
  - [x] Markdown table と Rich table の対応範囲を決める。
  - [x] Rich 側で table 挿入・行列操作をどこまで提供するか決める。
  - [x] 旧 table 操作 UI / CSS の残存箇所を確認し、再設計時に整理する。
  - [x] table の中に table を作れない制約を維持する。
  - [x] 確定した仕様に沿って Rich 側の table 操作 UI を実装する。
  - [x] 未使用の旧 table 操作用 CSS を実装時に整理する。
  - [x] table 選択中の行・列操作 UI を実機で確認する。
  - [x] table 作成済みノートで Markdown / Preview 往復を実機で確認する。
  - [x] 別ノートから table 作成済みノートを開いて内容が消えないことを実機で確認する。
- serializer の継続確認
...
- 作業整理
  - [ ] 今回のエディタ変更をコミット候補として整理する。
  - [ ] 変更済みファイルと未追跡ファイルを確認し、不要な差分が混ざっていないか確認する。
