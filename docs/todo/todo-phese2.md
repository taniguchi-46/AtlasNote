...
- MVPの移行前必須項目とCI確認は完了済み。Phase 2 CIの記録は、GitHub Actionsの[CI run #29383600495](https://github.com/taniguchi-46/AtlasNote/actions/runs/29383600495)を参照する。
- 検索入力UI、検索API、SQLite FTS5索引、外部Markdown reconciliationは実装済み。
- ノートブック、お気に入り、ピン留め、ゴミ箱は実装済み。
- タグCRUD、ノートとの多対多関連、タグクリックによる一覧遷移、ノート側のタグ選択ポップアップ、バックリンクは実装済み。タグ名の候補検索・検索APIへのタグ条件は廃止済み。全文検索索引は実装済み。関連メモはPhase 4へ完全移管し、Phase 2の対象外とする。
...
- [x] Phase 2実装時点の作業ツリーに、Phase 2実装と無関係な変更がないことを確認する。
...
- [x] ノートリンクの記法を決める（`atlasnote://note/<32桁小文字hex ID>`を使うMarkdownリンク）。
- [x] バックリンクの抽出規則を決める（コードフェンス・インラインコード・画像・外部URLを除外し、重複を除去する）。
- [x] 存在しないリンク先、タイトル変更、ノート削除時の扱いを決める（Markdownは保持、タイトル変更はリンクを維持、削除時は索引から除去）。
- [x] バックリンクを実装する。
- [x] 循環リンク、大量リンク、不正なリンク記法をテストする。

## 8. テーブルコピー

- [x] コピー対象を表全体に統一する（セル・行・列単位の独自コピーは対象外）。
- [x] `text/plain` にMarkdown、Rich貼り付け用の`text/html`に表構造を出力する方針を決める。
- [x] Markdownを壊さないテーブルコピーを実装する。
- [x] Markdown / Rich往復後も表の内容が保持されることをテストする。
- [x] 空セル、改行、インライン装飾、特殊文字をテストする。
...
実装済みの記録と、CI・手動確認の最終結果は分けて扱う。CIで確認できた条件はチェックし、Phase 3へ持ち越す残課題は下記の扱いを維持する。

- [x] CIに含まれる正常系、異常系、境界値テストが成功する。
- [x] CIに含まれる検索、タグ遷移、並び替えのテストで結果が一貫する。
- [x] CIに含まれるSQLインジェクション対策とMarkdown安全性、構造化ログのテストが成功する。
- [x] CIに含まれる既存の保存、編集、ノートブック関連テストが成功する。
- [x] CIのGo testsでmigration、既存データ不変性、rollback、将来version拒否を確認するテストが成功する。
- [x] CI run時点の実装状態と関連ドキュメントが一致する。
- [x] CIの全チェックが成功する。

### 残課題のPhase 3への扱い

- [ ] 競合解決UIのコンポーネントテストは、Phase 3の競合UI実装完了前に追加する。
- [ ] 大量ノート性能の継続比較は、Phase 3の大量同期・一覧更新の受け入れ時に、同じ環境と件数条件で実施する。
- [ ] Rich機能追加時のserializer round-tripテストは、Rich serializerを変更するPhaseで対応する。Phase 3同期の開始条件にはしない。
