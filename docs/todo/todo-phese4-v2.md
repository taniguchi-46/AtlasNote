...
- `【要仕様整合】`: 利用者要求に基づく実装はあるが、既存scopeの記述を更新して正本を一致させる必要がある項目。
...
- [ ] 【要仕様整合】Web検索は今回の利用者要求に基づき単一チャットの`＋`メニューへ実装した。一方、v2 scopeの「AIエージェント、外部サービス連携」は対象外とする記述との整合を、Web検索の安全契約（明示確認、OpenRouter Web Searchのみ、結果を候補として扱う）を含めて更新する。
- [ ] 【要仕様整合】UI上の`Agent`は、承認済みのローカル読み取り・候補生成だけを行うモードであり、scopeで対象外とする任意のAIエージェントや外部サービス操作は追加していない。この用語上の区別をv2／v3 scopeへ明記する。
