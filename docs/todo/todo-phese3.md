...
- 設計レビューは完了している。
- WebDAV通信、同期用migration、認証情報の保存処理は未実装である。
- Phase 2のCI受け入れ条件と残課題の扱いを確認してから、Phase 3の実装開始を確定する。
...
- [x] 手動同期、自動同期（5秒debounce・5分poll）、オフライン、timeout、部分成功時の状態遷移を決定する。
...
- [ ] 同期開始前に、Phase 2の対象テストを含むCI受け入れ条件を確認する。

### 3. 設計承認後に実装・検証する

- [x] 設計レビュー内容を、`docs/development/implementation-plan.md` のPhase 3順序へ反映する。
- [ ] 実装後、正常系・異常系・競合・クラッシュ復旧・データ保全をテストする。
- [ ] 手動同期、自動同期、同期競合解決のUI状態を確認する。
- [ ] 関連ドキュメント、TODO、CIを実装状態へ更新する。
