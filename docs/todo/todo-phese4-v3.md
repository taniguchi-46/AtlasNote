...
v3の保存基盤、AIアシスタント／ライティングの基本経路、明示保存・削除・stale／orphaned境界、単一チャットtimeline、Provider管理Web検索、共通下書き、制限付きAgentの本文差分提案・編集権限設定（既定の明示適用と検証済み自動適用）、保存成功後のエディタ即時反映、v3相当のFrontend／Goテストは確認済みである。Wails公開APIからService、Repository、一時SQLiteまでの履歴・成果物ライフサイクル、version 10既存データ保持、version 12→13再構築失敗時のrollback、Provider失敗後のローカル機能継続も2026-08-23に自動検証した。2026-08-24に空結果・候補なし・長文のFrontend通し異常系と、Assistant／Agentのrequest ID相関による利用者停止を追加した。commit `6e7a2dfbd6e8f4a27f74355460714951c7db8b7d`までの差分はCI run [#32666344532](https://github.com/taniguchi-46/AtlasNote/actions/runs/32666344532) で成功している。v2完了条件に残る大量ノート・候補採用・全保存境界の追加検証、手動受け入れ、後続ローカル差分のCI反映は未完了として残す。
...
- [x] 【自動検証済み】空結果、候補なし、長文、対象ノート切替、revision差異、timeout、cancelを通しでテストした。Goでは候補なし、長文上限、timeout分類、外部context cancelと利用者cancel時のProvider停止・安全なエラー・生成lock解放、wrong／stale／terminal済みrequest ID拒否、終了との競合、Agent提案・履歴の非永続を確認した。Frontendでは対象ノート切替、事前revision差異、差分不一致、保存時CAS競合、保存失敗、AI司書の利用者cancel／timeout、Assistant／Agentの`AI_TIMEOUT`／`AI_CANCELLED`応答、Writing timeout、AI司書の正常な候補0件、要約・Assistant・Writingの空／無効応答、長文contextの切り詰めmetadata、`AI_INPUT_TOO_LARGE`時の安全な非保存を確認した。Assistant／Agentの停止はcontext準備中のProvider非呼び出し、生成中のexact-ID停止、`canceling`中の送信lock、停止API失敗後の終端監視、下書き・user entry保持、Agent提案・自動適用・履歴保存・自動retryなし、停止後の明示再試行成功まで実StoreとUI契約で検証した（2026-08-24）。
