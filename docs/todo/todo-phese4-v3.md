...
- `【要仕様確認】`: 既存scopeと今回の単一チャット方針に差異があり、実装を追加するか仕様を更新するかの判断が必要な項目。

v3の保存基盤、AIアシスタント／ライティングの基本経路、明示保存・削除・stale／orphaned境界、単一チャットtimelineへの統合、v3相当のFrontend／Goテストは確認済みである。手動受け入れ、Wails APIの個別通し確認、追加異常系、最新差分のCI記録は未完了として残す。
...
- [ ] 【要仕様確認】v3 scopeは機能ごとに分離した下書きを要求するが、単一チャット化ではChatGPTライクな共通下書きへ統合している。機能別下書きを追加するか、共通下書きを正としてscopeを更新するかを決定する。
