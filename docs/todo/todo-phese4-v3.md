...
決定状態: 正式決定（2026-07-27）／v3保存仕様確定（2026-07-28）／進捗分類更新（2026-08-01）
...
v3の保存基盤、AIアシスタント／ライティングの基本経路、明示保存・削除・stale／orphaned境界、単一チャットtimeline、Provider管理Web検索、共通下書き、v3相当のFrontend／Goテストは確認済みである。制限付きAgentの変更提案・差分確認・明示適用、手動受け入れ、Wails APIの個別通し確認、追加異常系、最新差分のCI記録は未完了として残す。
...
- [ ] 【決定済み・未実装】制限付きAgentモードで、利用者が明示選択した1ノートの本文変更案を作成する。タイトル・タグ・所属Notebook・リンクは既存の個別候補採用フローを維持する。対象ノート、基準revision、変更理由、変更前後またはdiffをtimelineの提案カードへ表示し、利用者が明示的に適用／破棄する。適用は既存のNote Service、revision/CAS、保存laneを通し、stale・競合・保存失敗・キャンセルでは変更を保存しない。
