...
- `SupportWorkspace`の整理／AIタブ切替、最小化、配置変更で、進行中のメモリ状態を意図せず破棄しない。AIタブはノート未選択時も表示できるが、既存の送信制約を維持する。ロック時は非表示タブの保護対象由来の内容を消去する。
...
- 共通サポートパネルは右側／下側配置とポインタ・キーボードによるresizeを維持する。AI内の新規チャット・履歴・設定操作はAIタブ内に置く。
