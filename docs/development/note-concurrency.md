...
- コンテンツロック前も、編集中の入力をエディタから回収してdraftへ反映し、保存成功後にだけロック処理へ進む。回収または保存に失敗した場合は鍵を破棄しない。
- コンテンツロックのStore busyは、入力回収・draft保存・API・Appの表示更新まで保持する。表示更新はStoreがawaitするコールバックで実行し、取得元エディタの入力ガードをfinallyで解除してからbusyを終了する。表示更新中の後続要求は保存前に拒否し、古い通知watcherの完了によるガード解除・新通知消失を防ぐ。表示更新失敗も入力ガードを解放し、成功済みのロック結果は維持する。
- fire-and-forgetで開始する保存Promiseにも必ず失敗処理を接続し、未処理Promiseを発生させない。

### 削除前準備と選択競合

- ゴミ箱への移動、完全削除、ゴミ箱を空にする操作は、対象IDを最初に削除準備中として記録し、対象エディタの最新入力を`flush(noteId)`してから同じノートlaneへ操作を投入する。
- flushをlaneへの削除投入より先に行う。autosaveが同じlaneの完了を待つ構造で削除側が先にlaneを占有しないため、保存待ちのデッドロックを作らない。
- flushが通常失敗またはrevision conflictになった場合は、対象ノートを削除せず、`failed`／`conflicted` draft、本文、アクティブ選択を保持する。成功した対象だけを部分成功として一覧から反映する。
- 更新または完全削除が成功した対象がアクティブなら、その対象だけを閉じる。全体の選択・ロック世代を無効化する`clearActiveNote`は使用せず、別ノートの最新選択とロック応答を取り消さない。
- 削除準備中の対象への選択とタイトル・本文・モード・属性操作を抑止し、進行中のNodeView／編集ダイアログの入力を保存しない。成功後にautosaveを再開して削除済み本文を復活させてはならない。
