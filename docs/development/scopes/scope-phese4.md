...
- AI関連機能はエディター内のAIワークスペースに集約する。ワークスペースは単一の会話・作業領域とし、下部の共通コンポーザーにある`＋`メニューから要約、AI司書、質問・壁打ち、ライティングを明示的に選択する。AI司書はサブメニューでタイトル、タグ、分類、関連メモ、重複候補を選び、要約と各AI司書操作は既存の送信前確認を経て実行する。質問・壁打ちとライティングは共通コンポーザーの入力を既存の各機能へ渡す。保存済み履歴・成果物はヘッダーの履歴アイコンから開き、要約とAI司書の一時結果は一覧へ含めない。機能切替・ワークスペースの開閉・履歴表示で進行中のAI状態を破棄しない。モデル名を表示するコンポーザーのモデル切替ボタンはAI設定画面を開き、保存済みAPI Keyを読み出さず、既存の下書き・接続確認・明示適用の流れでのみモデルを変更する。既定は右側、設定で下側へ配置を切り替える。右側の幅（300〜960px）と下側の高さ（180〜760px）は既存3ペインと同じドラッグ操作で調整し、右側設定を画面幅に応じて下側へ自動変更しない。保存した幅・高さは利用者の希望値として保持し、狭いウィンドウでは表示時だけ実効寸法を縮小する。AIアシスタントとAIライティングの共通入力欄・追加設定は、パネル幅に応じて狭いときに1列へ切り替える。エディターツールバーの位置対応アイコンで開閉し、配置・寸法設定だけを端末のUI設定として保存する。AI入力・生成結果・API Keyを`localStorage`へ保存しない。
- `＋`メニューの`要約`から要約を実行する。設定・接続確認・モデル選択が未完了の場合、ゴミ箱内ノート・空本文・12 KiB超過本文の場合、または既存draftの保存が失敗・競合した場合は、外部送信せずに理由を表示する。機密ノートの自動判定・自動マスキング・自動切詰め・分割・バッチ処理は行わない。
