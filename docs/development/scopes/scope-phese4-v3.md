...
決定状態: 正式決定（2026-07-27）／v3保存仕様追補確定（2026-07-28）／単一チャット・Provider管理Web検索・制限付きAgent方針確定（2026-08-01）
...
- Agentは制限付きAgentモードとして、利用者が明示選択した1ノートの本文変更案を作成できる。タイトル・タグ・所属Notebook・リンクは既存の個別候補採用フローを維持し、Agentの本文差分適用対象に含めない。変更案は差分確認カードへ表示し、利用者の明示適用時だけ既存のrevision/CAS・保存lane・Note Serviceを通す。自動保存、全ノートへの一括変更、任意コマンド実行は行わない。これは決定済みの追加範囲であり、書き込み・差分確認UIは未実装としてv3 TODOで追跡する。
