...
決定状態: 正式決定（2026-07-27）／v3保存仕様追補確定（2026-07-28）／単一チャット・Provider管理Web検索・制限付きAgent方針確定（2026-08-01）／Agent差分提案・明示適用実装（2026-08-02）
...
- Agentは制限付きAgentモードとして、開いている1ノートの本文変更案を作成できる。タイトル・タグ・所属Notebook・リンクは既存の個別候補採用フローを維持し、Agentの本文差分適用対象に含めない。変更案は基準revisionと一意な`before → after`本文差分の確認カードへ表示し、利用者の明示適用時だけ既存のrevision/CAS・保存lane・ノート更新APIを通す。自動保存、全ノートへの一括変更、任意コマンド実行は行わない。追加コンテキストは読み取り専用とする。
...
- Agentの変更提案は、対象ノート、基準revision、変更理由、変更前後またはdiff、変更フィールド、適用／破棄を表示し、stale・競合・保存失敗ではノートを変更しないことを検証する。

## 対象外

- Groq、OpenAI、Ollama、LM StudioのProvider追加。追加ProviderはPhase 4完了後の別スコープとする。
- AIデータのWebDAV同期、端末間のAI履歴共有、クラウド履歴サービス。
- 任意のAIエージェント、任意コマンド実行、任意の外部サービス操作、添付ファイル・画像・音声のAI連携。明示確認付きのProvider管理Web検索と、差分確認・明示適用に限定したAgent変更提案は本scopeの対象内とする。
...
- 制限付きAgentの変更提案、差分確認、明示適用／破棄、revision/CAS・競合・保存失敗境界がFrontend／Go／手動受け入れで成功している。
