...
- 図の表示領域は内部スクロールを持ち、NodeViewと編集ダイアログで10〜200%の表示倍率変更を提供する。現在の100%は従来の50%相当の表示サイズとし、倍率は表示専用で、本文・SQLite・同期・バックアップへ保存しない。表示ハンドルを上下方向にドラッグして倍率を変更し、図上の右クリックでソース編集ダイアログを開く。
- ツールバーのMermaid挿入はRichでは既存`codeBlock`へ、Markdownでは安全なMermaidフェンスへ変換する。Mermaidフェンスの入力ルールは通常コードフェンスと区別し、直後の空段落へ編集位置を移す。
