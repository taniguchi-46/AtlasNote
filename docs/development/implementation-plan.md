...
- 正常系、強いETag、outbox/base snapshot、TLS/proxy、資格情報境界、フェイルセーフ、412、復旧rollback、同期ストア、既存回帰テスト、別名出力のWails本番build、秘密情報非露出の自動検証を完了。非本番の実WebDAV相互運用を受け入れ確認済み。
- 手動同期、自動同期、空状態、失敗状態、競合解決のUIを手動確認済み。実サーバーまたは同期実装の更新時は同じ非本番環境で回帰確認する。
