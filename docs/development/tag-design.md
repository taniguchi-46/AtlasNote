...
タグのデータモデル、制約、migration、API責務は確定済みです。migration、Repository、Service、Wails API、Store、UIは未実装です。
...
- 現在のschema version 5の後ろへ、上記2テーブルとINDEXを作成するmigration version 6を追加する。
- `migrations`と`ensureCompatibleSchema`の両方へ同じタグDDLを反映し、既存の自己修復パターンに合わせる。
- 既存の`migrate`は1トランザクション内でDDLを実行し、成功時だけ`PRAGMA user_version = 6`を確定する。この方式を維持する。
- 既存の`notes`、Markdown、`revision`、日時、FTS5索引、検索状態を更新・backfillしない。追加直後の`tags`と`note_tags`は空である。
...
設計上は空テーブル追加だけであり、既存ノート・Markdown本文・ノートブック・検索索引にデータ変換は発生しない。migration実装時はversion 5 DBをfixtureとして、既存レコードが不変であることを検証する。

## 実装時の必須テスト

- 空文字、NUL、制御文字、100文字、101文字、Unicode空白、NFC差異、大文字小文字の入力検証
- 同名作成・改名時の`TAG_NAME_CONFLICT`、同一タグへの大小文字だけの改名
- 未存在ノート・タグへの関連付け、重複関連、外部キー違反
- タグ削除でノートが残ること、ノート完全削除でタグが残ること、ゴミ箱移動・復元で関連が残ること
- `SetNoteTags`の全解除、重複ID除去、トランザクション失敗時の全体rollback
- version 5 DBのmigration、migration失敗時のrollback、将来version DBの拒否
- タグ名・IDをSQL文字列へ連結せず、ログへタグ名やノート本文を出さないこと

## 実装順序

1. schema version 6とdatabaseテストを追加する。
2. Tagモデル、Repository、Service、タグ単体テストを追加する。
3. Wails API、生成bindings、フロントAPI、Pinia Storeを接続する。
4. タグ作成・編集・削除・付与・解除のUIと回帰テストを追加する。
5. 検索・フィルター設計を拡張し、タグ条件を既存検索APIへ統合する。
