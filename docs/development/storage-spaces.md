...
`ATLAS_NOTE_DATA_DIR` は個別の保存空間ではなく、Atlas Noteが管理する保存ルートを示す。保存領域とバックアップ保存領域の選択、初回起動、移行は [`storage-locations.md`](storage-locations.md) を参照する。
...
- 保存空間の一覧・作成・選択は「設定 > 保存場所」の下部にある保存空間設定だけから行う。トップバーには選択UIを置かない。
...
- 保存空間の削除、改名、保存空間ごとの外部フォルダ選択は提供しない。物理保存場所の変更と保存空間別の本文ロックは「設定 > 保存場所」で設定し、ロック状態の保存空間を次回起動時に解除できる。
