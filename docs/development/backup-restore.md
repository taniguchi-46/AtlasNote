...
アクティブな保存空間のSQLiteメタデータとMarkdown本文を、設定されたアーカイブルートへ自動保存する。保存空間の切替、手動インポート、クラウド同期はこの機能の範囲に含めない。物理保存場所の選択と移行は [`storage-locations.md`](storage-locations.md) を参照する。

## 保存場所と内容

アーカイブルートから次の固定パスへ保存する。アーカイブルートがデータルートと同じ場合は、従来どおりデータルート内に作成される。
...
アーカイブルートがデータルートと異なる場合、復元の一時領域はアーカイブと分離し、次のデータルート配下へ置く。

```text
<data-root>/.atlasnote-restore/<spaceID>/
  staging/...
  rollback/...
  pending.json
```

アーカイブルートとデータルートが同じ場合は、従来どおり`.atlasnote-backups/<spaceID>/`配下の一時領域を使用する。
外部アーカイブを取り外したままでも検証済みstageは保持されるが、復元前の安全用バックアップを作成する適用時にはアーカイブルートを再接続する必要がある。
...
- バックアップ用のWails APIはファイルパスや内部管理領域をフロントエンドへ返さず、エラーは構造化された安全なメッセージへ変換する。保存場所設定APIは選択結果の表示に必要なルートだけを返す。
