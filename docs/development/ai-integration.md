...
- 決定内容:
  ...
  - AIワークスペースは単一の会話・作業領域とし、下部共通コンポーザーの`＋`メニューから要約、AI司書、質問・壁打ち、ライティングを明示的に選択する。AI司書はタイトル、タグ、分類、関連メモ、重複候補のサブメニューから選ぶ。要約とAI司書操作は既存の送信前確認を経て実行し、質問・壁打ちとライティングは共通コンポーザーの入力を既存の各機能へ渡す。保存済み履歴・成果物はヘッダーの履歴アイコンから開き、要約とAI司書の一時結果は一覧へ含めない。機能切替・ワークスペースの開閉・履歴表示で進行中のAI状態を破棄しない。コンポーザーのモデル切替ボタンは現在のモデル名を表示してAI設定画面を開き、保存済みAPI Keyを読み出さず、既存の下書き・接続確認・`適用`/`OK`でのみモデルを変更する。既定は右側、設定で下側へ配置を切り替える。右側の幅（300〜960px）と下側の高さ（180〜760px）は既存3ペインと同じドラッグ操作で調整し、右側設定を画面幅に応じて下側へ自動変更しない。保存した幅・高さは利用者の希望値として保持し、狭いウィンドウでは編集領域を確保するため表示時だけ実効寸法を縮小する。AIアシスタントとAIライティングの共通入力欄・追加設定は、AIワークスペースの幅が狭いときに1列へ切り替える。エディターツールバーの位置対応アイコンで開閉し、ワークスペース内のメニュー・履歴・閉じる・操作ボタンはタイトルとアクセシブル名を持つ。配置・寸法設定と非秘密のAgent本文編集権限を端末のUI設定として保存し、AI入力・生成結果・API Keyを`localStorage`へ保存しない。AI設定・接続確認・モデル選択が未完了の場合は外部送信せず、AI設定タブを開く。ゴミ箱内ノート、空本文、UTF-8で12 KiBを超える本文も送信せず、本文の自動マスキング・切詰め・分割・バッチ処理を行わない。機密ノートの自動判定はせず、送信前確認で利用者が判断する。
...
- 決定内容:
  ...
  - GoのApplication Service・CredentialStore・Wails APIテストで、プロバイダー別資格情報、session-only fallback、更新・削除、失敗時の非保存、同時要約1件、要約前の保存成功/失敗/競合、`baseRevision`、安全エラーだけの返却を検証する。合成した秘密情報・本文・raw provider error markerがAPI応答、エラー、ログ、設定、SQLite、Markdown、操作journal、WebDAV outboxに現れないことを確認し、app testではSQLite本体・WAL/SHM、設定・復旧ファイル、クラッシュ成果物を含むアプリデータディレクトリ全体を走査する。
  ...
  - フロントエンドは既存のNodeスクリプト方式に合わせ、mock Wails APIを差し替える`test:ai-store`を追加する。AI設定下書き、接続確認失敗時の非保存、モデル再選択、送信前確認、保存済みsnapshot、単一実行、状態遷移、手動retry、`Retry-After`待機表示、切替後の結果破棄、stale表示、コピーだけ、通知・ログ非露出を検証し、AI Storeの`localStorage`アクセスがないことを動的に検証する。Vueコンポーネント用の新規テスト依存は追加しない。
...
- 完了したAIアシスタント会話は、利用者の追加保存操作を待たず、最初の完了済みuser／assistantペアからアクティブ保存空間のSQLiteへ自動保存する。続きの質問は同じ`ai_histories.id`を更新し、「新しいチャット」は別IDの会話として開始する。
- 履歴保存に失敗しても画面上の会話とProvider応答は保持し、送信を再実行せずに履歴保存だけを再試行する。保存・読込・削除・新規チャットは世代とbusy状態を確認し、古い非同期応答で現在の会話を上書きしない。
- 履歴一覧は全件を取得する。履歴のmessageは読み込み時にそのまま再表示し、次回送信時の会話コンテキストとして利用する。tool trace、system／内部指示、raw context、資格情報、raw provider error、WebDAV同期は引き続き保存対象外とする。
...
- 要約はモデルのコンテキスト長を考慮して送信上限を判定し、固定12 KiBで拒否しない。本文の自動切詰め・分割は行わない。要約とAIアシスタントの回答はMarkdownを正規出力とする。Providerまたは既存履歴にHTMLが含まれる場合も、AI専用プレビューでMarkdownとHTMLを解析し、DOMサニタイズ後の要素だけを表示する。`script`、イベント属性、危険なURL、画像などの外部リソースは許可せず、保存済み原文は書き換えない。通常ノートのraw HTML無効化は維持する。
- AIライティング結果は、利用者の明示確認後に新規ノートとして作成するか、生成時の参照revisionと一致する現在ノートへ追記または本文置換できる。反映は既存のrevision/CAS・ノート単位保存laneを通す。
- Agent本文編集権限は端末ローカルの非秘密UI設定であり、既定の`review-required`では提案カードから明示適用する。`auto-update`では外部送信前の確認後、通常のAgent送信が返した検証済み本文1差分だけを既存のrevision/CAS・ノート単位保存laneで自動適用し、変更前後は現在のtimelineに残す。Agent + Web検索、メタデータ変更、バッチ変更、任意コマンド実行には適用しない。
- AI司書と制限付きAgentでは、モデルの構造化出力能力を`対応`／`非対応`／`不明`の三値で扱い、モデル一覧を補助情報として実行時応答を正とする。OpenRouterの`stream`はモデルの`supported_parameters`から判定せず、構造化出力パラメーターの情報がない場合はProviderへ委譲する。Geminiのモデル一覧だけでは構造化出力の可否を確定できないため、明確な非対応だけを送信前に拒否する。非対応モデルは安全な型付きエラーを返す。

### 接続診断・保存済み資格情報・モデル切替の追補（2026-08-01）

- 認証確認（`CheckConnection`）、モデル一覧取得（`ListModels`）、実生成（`TestGeneration`／各AI機能）は別の通信として扱う。認証確認または一覧取得の成功だけでは、選択モデルの`generateContent`成功を保証しない。
- AI設定は、API Key入力欄が空の場合に限り、利用者が明示した保存済み資格情報を認証確認・モデル一覧取得・生成確認に再利用できる。保存済みキーは表示・Wails応答・SQLite・同期データへ出さない。新しいキーを入力して適用した場合だけ既存キーを置き換える。
- 保存済み資格情報が利用可能な場合、利用者は別途の認証確認やAPI Key再入力を行わず、明示的なモデル一覧更新、モデル選択、適用／OKでモデルだけを切り替えられる。モデル一覧の自動取得・自動選択は行わない。
- `適用`／`OK`で確定したプロバイダーは、非秘密の選択状態として端末ローカルSQLiteへ保持する。再起動または設定画面の再表示後も同じプロバイダーとモデルを復元し、未適用のドロップダウン変更ではAI実行先を変えない。
- 選択済みプロバイダーが再認証必要または利用不能になった場合は、別プロバイダーへ自動切替せず、再認証または明示的な再選択を求める。選択状態はAI資格情報・モデル設定と同様にWebDAV同期しない。
- モデルだけを切り替える場合は、既存資格情報の参照と保存方式を変更せず、ローカルの`model_id`と選択済みプロバイダー状態だけを更新する。API Key、WebDAV同期対象、モデル一覧の自動取得は追加しない。
- 「生成を確認」は利用者の明示操作でのみ、短い固定テキストを選択モデルへ一度送信して結果を破棄する。ノート本文・生成結果は保存・同期せず、自動実行もしない。プロバイダーの無料枠を含む利用量を消費する可能性がある。
- Geminiのモデル一覧取得、生成、ストリーミングは、同じ`v1beta` RESTリソースへ送信する。認証確認だけ`v1`、生成だけ`v1beta`のようにAPIバージョンを混在させない。
- `gemini-3.6-flash`の要約とAI司書は、固定出力上限を既定の内部思考だけで消費しないよう`thinkingLevel`を`minimal`にする。出力上限は増やさず、他モデルやAIアシスタント・ライティングの思考設定は変更しない。
- Geminiの構造化エラー状態`FAILED_PRECONDITION`、出力上限、コンテンツブロックは型付き安全エラーへ正規化する。providerの自由文メッセージ・error body・資格情報はUI、Wailsエラー、ログへ返さない。
