...
保存場所の再起動時移行失敗を`storage-recovery`へ接続し、元へ戻す／別の空フォルダへ切り替える／同じ移行を再試行する保留操作、候補の再検証、Windowsインストーラーの日本語化・安全なアンインストールを実装済み。Go・フロントエンド・Wails本体ビルドは確認済みで、NSIS実体は`makensis`未導入のため未検証（2026-09-05）。
