...
- AI機能は`AIWorkspace`の下部共通コンポーザーにある`＋`メニューから明示選択し、機能切替、履歴表示、ワークスペースの開閉で進行中のAI状態を破棄しない。モデル切替ボタンは既存のAI設定画面を開き、API KeyやAI入力・結果をUI設定へ保存しない。保存したワークスペース寸法は狭いウィンドウでも希望値として保持し、表示時だけ実効寸法を調整する。
