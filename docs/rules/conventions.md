...
- Mermaidは既存`codeBlock`のソースを正本とし、Richでは図のみを表示して編集ダイアログからソースを更新する。かんたん編集の要素モデルは表示中だけの一時データで、未対応の行は原文を保持してソース編集へ引き継ぐ。生成SVGは表示専用の一時データとし、保存・同期・エクスポートのデータモデルへ追加しない。外部リソースを伴う記法は許可しない。
