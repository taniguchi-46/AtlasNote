...
- AI機能は`AIWorkspace`の単一チャットtimelineへ統合する。コンポーザーでは開いているノートを削除不能の固定context chipとして表示し、`＋`メニューから追加ノート、Notebook検索scope、要約、文章作成6種、タイトル・タグ・分類・関連・重複・Web検索のスキル／ツールを選択する。Askは読み取り専用、Agentも許可済み読み取りと候補生成だけに限定し、変更は明示採用後に既存の保存境界を通す。送信ボタンは入力欄の右下内側へ置く。
