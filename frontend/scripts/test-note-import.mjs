...
try {
  ...
  assert.match(apiSource, /recordNumber/, 'structured import results must identify a failing record')
  ...
  assert.match(modalSource, /JSON\/CSV/, 'the modal must explain structured imports')
  assert.match(modalSource, /title,content/, 'the modal must document the CSV columns')
