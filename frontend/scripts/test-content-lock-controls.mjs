...
try {
  ...
  for (const target of [
    ...
    assert.equal(editorToggle.getAttribute('aria-label'), 'ロック', 'toggle must retain an accessible name')
    assert.ok(editorToggle.querySelector('.content-lock-toggle-track'), 'toggle must render a visible switch track')
    assert.ok(editorToggle.querySelector('.content-lock-toggle-thumb'), 'toggle must render a movable switch thumb')
    assert.doesNotMatch(editorToggle.textContent, /オン|オフ/, 'toggle must not render on/off text')
    assert.doesNotMatch(mounted.container.textContent, /トグルを有効にすると、パスフレーズ欄が表示されます。/, 'idle state must not render redundant guidance')
    editorToggle.click()
    await flushUI()
    assert.equal(editorToggle.getAttribute('aria-checked'), 'true', target.type + ' toggle must turn on')
    assert.doesNotMatch(editorToggle.textContent, /オン|オフ/, 'enabled toggle must not render on/off text')
    assert.doesNotMatch(mounted.container.textContent, /パスフレーズを入力し、下の保存でロックを有効にします。/, 'enabled state must not render redundant guidance')
