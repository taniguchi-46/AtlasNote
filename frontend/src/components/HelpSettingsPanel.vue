...
const helpSections: readonly HelpSection[] = [
  ...
  },
  {
    ...
    title: 'アンインストール',
    paragraphs: [
      'アプリのアンインストールはWindowsの「インストールされているアプリ」から行います。追加削除の「端末の表示設定・キャッシュ」と「この利用者のAtlas Note用認証情報」は既定で選択されていません。',
      'アンインストールしても、ノート、バックアップ、保存空間、復旧情報、保存場所の管理情報は削除しません。追加削除を選んだ場合も、現在の利用者に確認できる既知の対象だけを処理します。使用中、復旧中、利用者を確認できない場合は安全のため中止し、アプリ終了後または正しい利用者で再試行してください。',
    ],
  },
]
</script>
